...
func (j *bindHandler[T]) Handle(ctx *Context) error {
	for _, opt := range j.opts {
		if err := opt(ctx); err != nil {
			return ctx.Failed(http.StatusBadRequest, http.StatusBadRequest, ErrBindParams, ctx.err)
		}
	}
	...
	if err := ctx.Bind(&t); err != nil {
		return ctx.Failed(http.StatusBadRequest, http.StatusBadRequest, ErrBindParams, err)
	}
	if err := ctx.Validate(&t); err != nil {
		return ctx.Failed(http.StatusBadRequest, http.StatusBadRequest, ErrBindParams, err)
	}
	if err := j.fn(ctx, t); err != nil {
		return ctx.Failed(http.StatusOK, http.StatusInternalServerError, ErrInternal, err)
	}
	return nil
}
