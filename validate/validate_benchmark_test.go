...
func BenchmarkApplyDefaultsOnly(b *testing.B) {
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		validator.applyDefaults(userReq)
	}
}
...
func BenchmarkApplyDefaultsNested(b *testing.B) {
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		validator.applyDefaults(productReq)
	}
}
...
func BenchmarkSetDefault(b *testing.B) {
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		validator.SetDefault(userReq, "Name", "默认名称")
	}
}
...
func BenchmarkSetFieldValue(b *testing.B) {
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		validator.setFieldValue(field, "测试名称")
	}
}
